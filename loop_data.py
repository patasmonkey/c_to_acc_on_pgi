...
def ipsea(ele,ll,kai):
    ...
    for e in ele.getiterator():
        if(nn+2<len(list(ele.getiterator()))):
            #print("cinp",e.tag,mae.tag,maemae.tag)
            nn+=1
            if(mae.tag=="arrayAddr"):
                #print("ainp",e.tag)
                if((mae.text in ll)==False):
                    ll.append(mae.text)
            if(mae.tag=="pragma" and maemae.tag!="pragma"):
                if(mae.tag=="pragma" and e.tag!="pragma" ):
                    #print("binp",e.tag)
                    mikke+=1
                    if(kai==mikke):
                        break
...
def cool(ele):
    ...
    for e in list(ele):
        ...
        if(mae=="pragma"):
            #print(e.tag,e.items(),e.text)
            ss=e.items()
            if(ss and e.tag!="linemarker"):
                if(ss[0][1]!="/opt/pgi/linux86-64/18.4/include/math.h"):
                    print("pra",ss[0][1],praco,e.tag,mae)
                    praco+=1;
        mae=e.tag
        cool(e)
...
def hatu_pra_or_loop(ele,n_root):
    ...
    if(ele.tag=="pragma" or ele.tag=="forStatement" or ele.tag=="whileStatement"):
        if(ele.tag=="pragma" and  "libm" in ele.text):
            return
        fz=1
        sub=ET.SubElement(n_root,'pragma')
        sub.set('lineno',str(yan+line2[sw])) 
